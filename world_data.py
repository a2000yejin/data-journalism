...
st.write("*옥수수 자급률 = 국내 옥수수 생산량/국내 옥수수 소비량")
st.text("")
...
st.text("나도 팝콘이 사라진 영화관을 상상하고 싶진 않아. 10년 뒤에도 팝콘을 먹으려면 우리 모두 기후위기에 관심을 가져야겠지? 물론 개인이 할 수 있는 노력이 많지는 않지만, 개개인의 행동이 바뀌어야만 정부와 기업의 기후위기 대응 속도가 달라질테니까!")
st.text("더 이상 기후위기는 나와 상관 없는 먼 미래의 일이 아니야. 오늘은 팝콘에 대해서만 이야기했지만, 지구의 온도가 올라가고 가뭄과 같은 이상 기후가 빈번하게 발생할 시 심각한 식량 부족 문제가 확대될 수 있다는 점만 알아줘.")
        
st.text("")        
st.text("맛있는 팝콘이 부디 지구에서 사라지지 않기를 🙏")
